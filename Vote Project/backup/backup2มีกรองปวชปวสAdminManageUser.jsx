...
export default function AdminManageUsers() {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:5000/api/users/levels')
            .then(res => res.json())
            .then(data => setEducationLevels(data.levels || []));
    }, []);
    ...
    const handleAddUser = async () => {
        try {
            ...
            if (data.success) {
                ...
                setFormData({ student_id: '', password: '', first_name: '', last_name: '', email: '', department_id: '', level_id: '', year_level: '', roles: [] });
                fetchUsers();
            } else {
            ...
            <div className="p-6 bg-gray-100 min-h-screen ">
                ...
                <div className="flex flex-wrap gap-4 mb-4 items-center">
                    ...
                    >
                        ...
                        {years
                            .filter(y => !selectedLevel || y.level_id === parseInt(selectedLevel))
                            .map((y) => (
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-3xl relative shadow-xl">
                        ...
                        <form className="grid grid-cols-2 gap-4">
                            <input name="student_id" value={formData.student_id} onChange={handleInputChange} type="text" placeholder="รหัสนักศึกษา" className="border border-purple-300 p-2 rounded" />
                            <input name="password" value={formData.password} onChange={handleInputChange} type="password" placeholder="รหัสผ่าน" className="border border-purple-300 p-2 rounded" />
                            <input name="first_name" value={formData.first_name} onChange={handleInputChange} type="text" placeholder="ชื่อ" className="border border-purple-300 p-2 rounded" />
                            <input name="last_name" value={formData.last_name} onChange={handleInputChange} type="text" placeholder="นามสกุล" className="border border-purple-300 p-2 rounded" />
                            <input name="email" value={formData.email} onChange={handleInputChange} type="email" placeholder="อีเมล" className="border border-purple-300 p-2 rounded" />

                            <select name="department_id" value={formData.department_id} onChange={handleInputChange} className="border border-purple-300 p-2 rounded">
                                <option value="">เลือกแผนก</option>
                                {departments.map((d) => (
                                    <option key={d.department_id} value={d.department_id}>{d.department_name}</option>
                                ))}
                            ...
                            <select name="level_id" value={formData.level_id} onChange={handleInputChange} className="border border-purple-300 p-2 rounded">
                                <option value="">เลือกระดับชั้น</option>
                                {educationLevels.map((l) => (
                                    <option key={l.level_id} value={l.level_id}>{l.level_name}</option>
                                ))}
                            ...
                            <select name="year_level" value={formData.year_level} onChange={handleInputChange} className="border border-purple-300 p-2 rounded">
                                <option value="">เลือกชั้นปี</option>
                                {years
                            ...
                            <div className="col-span-2 flex gap-4 items-center flex-wrap">
                                {['นักศึกษา', 'ผู้สมัคร', 'กรรมการ', 'ผู้ดูแล'].map((role) => (
                                    <label key={role} className="flex items-center gap-1">
                                        <input type="checkbox" checked={formData.roles.includes(role)} onChange={() => handleCheckboxChange(role)} /> {role}
                                    </label>
                                ))}
                            </div>
